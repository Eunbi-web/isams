...
class ApplicationController extends Controller {
    public function index(Request $r) {
        ...
        $applications = ScholarshipApplication::with(['student','scholarship'])
            ...
            ->latest()->paginate(25);
        return view('scholarship.applications.index',compact('applications'));
    }
    public function create() { $students=Student::orderBy('last_name')->get(); $scholarships=Scholarship::where('status','Active')->get(); return view('scholarship.applications.create',compact('students','scholarships')); }
    public function store(Request $request) {
        ...
        $app->save();
        return redirect()->route('scholarship.applications.index')->with('success',"Submitted! AI Score: {$result['score']}% — {$result['eligibility']}");
    }
    public function show(ScholarshipApplication $application) { $application->load(['student','scholarship']); return view('scholarship.applications.show',compact('application')); }
    public function edit(ScholarshipApplication $application) { $students=Student::orderBy('last_name')->get(); $scholarships=Scholarship::all(); return view('scholarship.applications.edit',compact('application','students','scholarships')); }
    public function update(Request $r, ScholarshipApplication $application) { $application->update($r->only(['status','remarks'])); return redirect()->route('scholarship.applications.show',$application)->with('success','Updated!'); }
    public function destroy(ScholarshipApplication $application) { $application->delete(); return redirect()->route('scholarship.applications.index')->with('success','Deleted.'); }
