...
<meta name="csrf-token" content="{{ csrf_token() }}"><title>ISAMS Student — @yield('title','Portal')</title>
...
<div id="studentNotifWrapper" style="position:relative;">
    ...
    <div id="studentNotifDropdown" style="display:none;position:absolute;top:calc(100% + 10px);right:0;width:360px;background:#fff;border:1.5px solid var(--bd);border-radius:14px;box-shadow:0 12px 40px rgba(0,0,0,.14);z-index:9990;overflow:hidden;">
        ...
        <div style="padding:11px 18px;border-top:1px solid var(--bd);background:var(--bg);text-align:center;">
            <a href="{{ route('student.notifications') }}" style="font-size:12px;color:var(--gm);font-weight:600;text-decoration:none;">View all notifications <i class="fas fa-arrow-right" style="margin-left:3px;font-size:10px;"></i></a>
        </div>
    </div>
</div>
...
// ── Student Notification Dropdown ────────────────────────────────────────────
(function(){
    var DROPDOWN_URL   = '{{ route("student.notifications.dropdown") }}';
    var MARK_ALL_URL   = '{{ route("student.notifications.mark-all") }}';
    var MARK_READ_BASE = '{{ url("student/notifications") }}';
