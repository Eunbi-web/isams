...
<div class="cb">
<div class="g2 mb3">
@foreach([['GWA',number_format($application->gwa,2),'fas fa-star'],['Enrollment',$application->enrollment_type,'fas fa-id-badge'],['Year Level',$application->year_level??'—','fas fa-graduation-cap'],['Academic Load',$application->academic_load??'—','fas fa-book-open'],['Academic Honors',$application->academic_honors??'—','fas fa-trophy'],['Application Type',$application->application_type??'—','fas fa-file-signature'],['Parent Employment',$application->parent_employment_status??'—','fas fa-briefcase'],['Siblings in College',$application->siblings_in_college??'0','fas fa-users'],['Has Failing',$application->has_failing?'Yes':'No','fas fa-exclamation'],['Has Discipline',$application->has_discipline?'Yes':'No','fas fa-gavel'],['Income Bracket',$application->income_bracket,'fas fa-hand-holding-usd'],['Filed',\Carbon\Carbon::parse($application->created_at)->format('M d, Y'),'fas fa-calendar']] as $d)
