{{-- Shared program details: used by the full view page AND the quick-view modal on the index --}}
@php
...
<div class="cb">
<div class="g2 mb3">
@foreach([['Benefit',$scholarship->benefits??'—','fas fa-gift'],['Amount (₱)',$scholarship->amount??'—','fas fa-money-bill-wave'],['Slots',$scholarship->slots??'—','fas fa-users'],['Source',$scholarship->source??'—','fas fa-university'],['Deadline',$scholarship->end_date?->format('M d, Y')??'—','fas fa-calendar'],['Slots Remaining',$scholarship->slots_remaining,'fas fa-user-check']] as $d)
...
@if($scholarship->description)<div class="alert al-i" style="margin-bottom:14px;font-size:13px;"><i class="fas fa-align-left"></i><span>{{ $scholarship->description }}</span></div>@endif
@if($scholarship->requirements)<div class="alert al-w" style="margin-bottom:14px;font-size:13px;white-space:pre-wrap;"><i class="fas fa-list"></i><span>{{ $scholarship->requirements }}</span></div>@endif
@if(!empty($criteria))
...
GWA of {{ $criteria['gwa_max'] ?? 1.75 }} or better &nbsp;·&nbsp; Family income not above ₱{{ number_format((float)($criteria['income_max'] ?? 400000)) }} per year &nbsp;·&nbsp; {{ ($criteria['no_failing'] ?? true) ? 'No failing grades' : 'Failing grades allowed' }} &nbsp;·&nbsp; {{ ($criteria['no_discipline'] ?? false) ? 'No disciplinary case' : 'Disciplinary cases allowed' }}</span></div>
@else
