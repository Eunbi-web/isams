...
@section('page-sub','Your system notifications')
@section('content')
<div class="card an">
<div class="ch">
...
<button type="button" onclick="counselorMarkAllRead()" class="btn btn-o btn-sm">
...
@endphp
<div style="display:flex;align-items:flex-start;gap:13px;padding:14px 20px;border-bottom:1px solid var(--bd);background:{{ !$notif->read ? 'rgba(77,184,122,0.06)' : 'transparent' }};transition:background .2s;" id="cnotif-{{ $notif->id }}">
...
@if(!$notif->read)
<button type="button" onclick="counselorMarkOneRead({{ $notif->id }})" style="background:none;border:1px solid var(--bd);border-radius:7px;padding:5px 11px;font-size:11px;color:var(--gm);cursor:pointer;font-weight:600;white-space:nowrap;flex-shrink:0;">
Mark read
</button>
@endif
</div>
...
function counselorMarkOneRead(id) {
    fetch('{{ url("counselor/notifications") }}/' + id, {
        method: 'POST',
        headers: {'X-CSRF-TOKEN': CSRF, 'X-Requested-With': 'XMLHttpRequest', 'Content-Type': 'application/json'},
        body: JSON.stringify({_method: 'PATCH'})
    }).then(function() {
        var row = document.getElementById('cnotif-' + id);
        if (row) {
...
function counselorMarkAllRead() {
    fetch('{{ route("counselor.notifications.mark-all") }}', {
        method: 'POST',
        headers: {'X-CSRF-TOKEN': CSRF, 'X-Requested-With': 'XMLHttpRequest', 'Content-Type': 'application/json'},
        body: JSON.stringify({})
    }).then(function() {
        if (window.isamsToast) isamsToast('All marked as read.', 'success');
