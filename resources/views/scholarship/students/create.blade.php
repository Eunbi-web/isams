...
<div style="max-width:700px;">
    <form method="POST" action="{{ route('scholarship.students.store') }}">@csrf
        <div class="card an mb3">
            <div class="ch"><i class="fas fa-user-graduate" style="color:var(--gm);"></i><h2>Student Information</h2></div>
            <div class="cb">

                {{-- Search helper (matches requirement: EDP/student_id or name autofill) --}}
                ...
                <div class="g2">
                    <div class="fg"><label class="fl">Course <span style="color:var(--danger);">*</span></label><select name="course" class="fc" required><option value="">Select course</option>@foreach(['BS Computer Science','BS Information Technology','BS Engineering','BS Education','BS Nursing','BS Business Administration','AB Communication','BS Psychology','BS Accountancy','BS Tourism'] as $c)<option value="{{ $c }}">{{ $c }}</option>@endforeach</select></div>
        ...
        <div style="display:flex;gap:10px;">
            ...
            <a href="{{ route('scholarship.students.index') }}" class="btn btn-o">Cancel</a>
        </div>
    </form>
</div>
...
function lookupStudent(){
    ...
    fetch('{{ route('scholarship.students.lookup') }}',{
        method:'POST',
