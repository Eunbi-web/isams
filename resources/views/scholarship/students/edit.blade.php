...
<div style="margin-bottom:14px;"><a href="{{ route('scholarship.students.show',$student->id) }}" class="btn btn-o btn-sm"><i class="fas fa-arrow-left"></i> Back</a></div>
<form method="POST" action="{{ route('scholarship.students.update',$student->id) }}">@csrf @method('PUT')
...
<div style="display:flex;gap:10px;"><button type="submit" class="btn btn-p"><i class="fas fa-save"></i> Update</button><a href="{{ route('scholarship.students.index') }}" class="btn btn-o">Cancel</a></div>
</form></div>
@endsection
