                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;gap:4px;">
                            @if(!$s->imported)
                            <form method="POST" action="{{ route('scholarship.scraper.import', $s->id) }}">@csrf
                                <button class="btn btn-p btn-sm btn-ic" title="Import to Programs"><i class="fas fa-download"></i></button>
                            </form>
                            <form method="POST" action="{{ route('scholarship.scraper.dismiss', $s->id) }}">@csrf
                                <button class="btn btn-o btn-sm btn-ic" title="Dismiss"><i class="fas fa-times"></i></button>
                            </form>
                            @else
                            <a href="{{ route('scholarship.programs.index') }}" class="btn btn-o btn-sm btn-ic" title="View in Programs"><i class="fas fa-eye"></i></a>
                            @endif
                            <form method="POST" action="{{ route('scholarship.scraper.destroy', $s->id) }}">@csrf @method('DELETE')
