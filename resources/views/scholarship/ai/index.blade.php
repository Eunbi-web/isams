...
<div class="card an mb3">
    ...
    <div class="cb">
        <form method="GET" id="aiFilterForm" style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;">
            ...
            <a href="{{ route('scholarship.ai.index') }}" class="btn btn-o btn-sm">Clear</a>
        </form>
    </div>
</div>
    ...
    <div class="tw"><table>
        ...
        <tr id="app-row-{{ $app->id }}" class="app-row" style="{{ $rowBg }}" data-id="{{ $app->id }}" data-scholarship-id="{{ $app->scholarship_id }}" data-scholarship-name="{{ $app->scholarship?->name ?? 'No Scholarship' }}" data-status="{{ $st }}">
            ...
            <td id="status-cell-{{ $app->id }}">
                <div class="st-wrap" style="display:inline-flex;align-items:center;gap:4px;position:relative;" data-url="{{ route('scholarship.ai.updateStatus',$app->id) }}">
            ...
            <td id="act-cell-{{ $app->id }}">
                <div style="display:flex;gap:4px;">
                    <a href="{{ route('scholarship.applications.show',$app->id) }}" class="btn btn-o btn-sm btn-ic" title="View"><i class="fas fa-eye"></i></a>
                    @if($uneval)
                    <button type="button" class="btn btn-ai btn-sm btn-ic eval-btn" title="Run AI evaluation" data-id="{{ $app->id }}" data-url="{{ route('scholarship.ai.runSingle',$app->id) }}" onclick="evaluateApp(this)"><i class="fas fa-robot"></i></button>
                    @endif
                    @if($st==='Pending')
                    <form method="POST" action="{{ route('scholarship.applications.approve',$app->id) }}">@csrf @method('PATCH')
                        <input type="hidden" name="status" value="Approved">
                        <button class="btn btn-s btn-sm btn-ic" title="Approve (AI Score: {{ $sc }}%)"><i class="fas fa-check"></i></button>
                    </form>
                    <form method="POST" action="{{ route('scholarship.applications.reject',$app->id) }}">@csrf @method('PATCH')
