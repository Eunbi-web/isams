...
<div style="margin-bottom:14px;display:flex;gap:8px;"><a href="{{ route('scholarship.applications.index') }}" class="btn btn-o btn-sm"><i class="fas fa-arrow-left"></i> Back</a></div>
...
<form method="POST" action="{{ route('scholarship.applications.updateStatus',$application->id) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="Approved"><button class="btn btn-s"><i class="fas fa-check"></i> Approve Application</button></form>
<form method="POST" action="{{ route('scholarship.applications.updateStatus',$application->id) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="Rejected"><button class="btn btn-d" onclick="return confirm('Reject this application?')"><i class="fas fa-times"></i> Reject</button></form>
<form method="POST" action="{{ route('scholarship.applications.updateStatus',$application->id) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="On Review"><button class="btn btn-o"><i class="fas fa-search"></i> Mark On Review</button></form>
</div></div>
@endif
</div>
@endsection
