...
<select name="status" class="fc" style="width:140px;" onchange="this.form.submit()"><option value="">All Status</option><option value="Pending" {{ request('status')==='Pending'?'selected':'' }}>Pending</option><option value="On Review" {{ request('status')==='On Review'?'selected':'' }}>On Review</option><option value="Approved" {{ request('status')==='Approved'?'selected':'' }}>Approved</option><option value="Rejected" {{ request('status')==='Rejected'?'selected':'' }}>Rejected</option><option value="Canceled" {{ request('status')==='Canceled'?'selected':'' }}>Canceled</option><option value="For Review" {{ request('status')==='For Review'?'selected':'' }}>For Review (legacy)</option></select>
...
<a href="{{ route('scholarship.applications.index') }}" class="btn btn-o btn-sm">Clear</a>
</form>
...
// CHANGE 1/5: badge colors for the full status set — Pending=b-w, On Review=b-i, Approved=b-s, Rejected=b-d, Canceled=b-gray
...
<td><span class="badge {{ $stC }}">{{ $app->status }}</span></td>
<td class="mono tm" style="font-size:11px;">{{ $app->created_at->format('M d, Y') }}</td>
<td><div style="display:flex;gap:4px;">
<a href="{{ route('scholarship.applications.show',$app->id) }}" class="btn btn-o btn-sm btn-ic"><i class="fas fa-eye"></i></a>
@if($app->status==='Pending')
<form method="POST" action="{{ route('scholarship.applications.approve',$app->id) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="Approved"><button class="btn btn-s btn-sm btn-ic" title="Approve"><i class="fas fa-check"></i></button></form>
<form method="POST" action="{{ route('scholarship.applications.reject',$app->id) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="Rejected"><button class="btn btn-d btn-sm btn-ic" title="Reject" onclick="return confirm('Reject this application?')"><i class="fas fa-times"></i></button></form>
@endif
</div></td>
</tr>
@empty
