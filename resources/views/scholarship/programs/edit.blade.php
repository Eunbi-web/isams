...
<div style="margin-bottom:14px;"><a href="{{ route('scholarship.programs.index') }}" class="btn btn-o btn-sm"><i class="fas fa-arrow-left"></i> Back</a></div>
@php $criteria = is_array($scholarship->ai_criteria) ? $scholarship->ai_criteria : json_decode($scholarship->ai_criteria ?? '{}', true); @endphp
<form method="POST" action="{{ route('scholarship.programs.update', $scholarship->id) }}">@csrf @method('PUT')
...
<div style="display:flex;gap:10px;">
    ...
    <form method="POST" action="{{ route('scholarship.programs.destroy', $scholarship->id) }}" style="margin-left:auto;">@csrf @method('DELETE')
