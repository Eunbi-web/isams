...
@section('page-title','Add Scholarship Program')
@section('page-sub','Create a new scholarship with full details and AI criteria')
@section('content')
<div style="max-width:760px;">
<form method="POST" action="{{ route('scholarship.programs.store') }}">@csrf
...
<div class="card an mb3">
    ...
    <div class="cb">
        ...
        <input type="text" name="name" class="fc" value="{{ old('name') }}" required placeholder="e.g. CHED Merit Scholarship Program"></div>
        <div class="g2">
            ...
            <option value="Private" {{ old('type')==='Private'?'selected':'' }}>Private</option></select></div>
        ...
        <div class="g2">
            ...
            <input type="text" name="source" class="fc" value="{{ old('source') }}" placeholder="Commission on Higher Education"></div>
            <div class="fg"><label class="fl">Total Slots</label>
            <input type="number" name="slots" class="fc mono" value="{{ old('slots') }}" placeholder="250" min="0"></div>
        </div>
        <div class="g2">
            <div class="fg"><label class="fl">Amount (₱)</label>
            <input type="number" name="amount" class="fc mono" step="0.01" value="{{ old('amount') }}" placeholder="40000.00"></div>
            <div class="fg"><label class="fl">Deadline</label>
            <input type="date" name="end_date" class="fc" value="{{ old('end_date') }}"></div>
        </div>
        <div class="fg"><label class="fl">Benefit Package <span style="color:var(--danger);">*</span></label>
        <input type="text" name="benefits" class="fc" value="{{ old('benefits') }}" required placeholder="Full Tuition + ₱2,000/month stipend + book allowance"></div>
        <div class="fg"><label class="fl">Description</label>
        <textarea name="description" class="fc" rows="3" placeholder="Brief description of the scholarship program...">{{ old('description') }}</textarea></div>
    </div>
</div>
...
<div class="card an mb3">
    ...
    <div class="cb">
        ...
        <div class="g2">
            ...
            <input type="number" name="ai_gwa_max" class="fc mono" step="0.01" min="1" max="5" value="{{ old('ai_gwa_max', 1.75) }}" placeholder="1.75">
            ...
            <input type="number" name="ai_income_max" class="fc mono" value="{{ old('ai_income_max', 400000) }}" placeholder="400000">
        ...
        <div class="g2">
            <label style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1.5px solid var(--bd);border-radius:var(--rs);cursor:pointer;background:var(--bg);">
                <input type="checkbox" name="ai_no_failing" value="1" {{ old('ai_no_failing',1)?'checked':'' }} style="margin-top:2px;accent-color:var(--g);width:15px;height:15px;flex-shrink:0;">
            ...
            <label style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1.5px solid var(--bd);border-radius:var(--rs);cursor:pointer;background:var(--bg);">
                <input type="checkbox" name="ai_no_discipline" value="1" {{ old('ai_no_discipline')?'checked':'' }} style="margin-top:2px;accent-color:var(--g);width:15px;height:15px;flex-shrink:0;">
...
<div style="display:flex;gap:10px;">
    ...
    <a href="{{ route('scholarship.programs.index') }}" class="btn btn-o">Cancel</a>
</div>
</form>
</div>
@endsection
