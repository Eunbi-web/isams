...
@section('page-sub','Manage and publish announcements visible to all students')
@section('content')
<div style="display:flex;justify-content:flex-end;margin-bottom:14px;"><a href="{{ route('scholarship.announcements.create') }}" class="btn btn-p btn-sm"><i class="fas fa-plus"></i> New Announcement</a></div>
...
<a href="{{ route('scholarship.announcements.edit',$ann->id) }}" class="btn btn-o btn-sm btn-ic"><i class="fas fa-edit"></i></a>
<form method="POST" action="{{ route('scholarship.announcements.destroy',$ann->id) }}">@csrf @method('DELETE')<button class="btn btn-d btn-sm btn-ic" onclick="return confirm('Delete?')"><i class="fas fa-trash"></i></button></form>
</div></td>
</tr>
@empty
