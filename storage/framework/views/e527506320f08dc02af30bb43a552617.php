<?php $__env->startSection('title','AI Scholarship Screening'); ?>
<?php $__env->startSection('page-title','AI Scholarship Screening'); ?>
<?php $__env->startSection('page-sub','Smart screening dashboard — rank physical applicants and generate approval recommendations'); ?>

<?php $__env->startSection('ai-bar'); ?>
<div class="ai-bar">
    <div class="ai-bar-label"><div class="ai-dot"></div>Groq AI Active</div>
    <div class="ai-bar-stats">
        <div class="ai-stat">Total Applications: <strong><?php echo e($stats['total']); ?></strong></div>
        <div class="ai-stat">Eligible: <strong><?php echo e($stats['eligible']); ?></strong></div>
        <div class="ai-stat">For Review: <strong><?php echo e($stats['review']); ?></strong></div>
        <div class="ai-stat">Not Eligible: <strong><?php echo e($stats['not_eligible']); ?></strong></div>
        <div class="ai-stat">Avg AI Score: <strong><?php echo e($stats['avg_score']); ?>%</strong></div>
    </div>
</div>
...
<div class="sg" style="grid-template-columns:repeat(5,1fr);margin-bottom:20px;">
    <div class="sc an d1"><div class="si t"><i class="fas fa-users"></i></div><div class="sv"><div class="lbl">Total Evaluated</div><div class="val"><?php echo e($stats['total']); ?></div></div></div>
    <div class="sc an d2"><div class="si g"><i class="fas fa-check-circle"></i></div><div class="sv"><div class="lbl">Eligible</div><div class="val"><?php echo e($stats['eligible']); ?></div><div class="chg">Score ≥ 75%</div></div></div>
    <div class="sc an d3"><div class="si o"><i class="fas fa-exclamation-circle"></i></div><div class="sv"><div class="lbl">For Review</div><div class="val"><?php echo e($stats['review']); ?></div><div class="chg">Score 50–74%</div></div></div>
    <div class="sc an d4"><div class="si r"><i class="fas fa-times-circle"></i></div><div class="sv"><div class="lbl">Not Eligible</div><div class="val"><?php echo e($stats['not_eligible']); ?></div></div></div>
    <div class="sc an d5"><div class="si y"><i class="fas fa-star"></i></div><div class="sv"><div class="lbl">Avg AI Score</div><div class="val"><?php echo e($stats['avg_score']); ?>%</div></div></div>
</div>
    ...
    <div style="padding:20px;">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;margin-bottom:14px;">
            ...
            <select id="screenScholarship" class="fc">
                ...
                <option value="<?php echo e($sch->id); ?>" data-name="<?php echo e($sch->name); ?>"><?php echo e($sch->name); ?></option>
    ...
    <div class="cb">
        <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;">
            ...
            <select name="eligibility" class="fc" onchange="this.form.submit()">
                ...
                <option value="Eligible"     <?php echo e(request('eligibility')==='Eligible'?'selected':''); ?>>Eligible</option>
                <option value="For Review"   <?php echo e(request('eligibility')==='For Review'?'selected':''); ?>>For Review</option>
                <option value="Not Eligible" <?php echo e(request('eligibility')==='Not Eligible'?'selected':''); ?>>Not Eligible</option>
            </select></div>
            ...
            <select name="scholarship_id" class="fc" onchange="this.form.submit()">
                ...
                <?php $__currentLoopData = $scholarships; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($sch->id); ?>" <?php echo e(request('scholarship_id')==$sch->id?'selected':''); ?>><?php echo e($sch->name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select></div>
            ...
            <select name="min_score" class="fc" onchange="this.form.submit()">
                ...
                <option value="90" <?php echo e(request('min_score')==='90'?'selected':''); ?>>90%+ (Top)</option>
                <option value="75" <?php echo e(request('min_score')==='75'?'selected':''); ?>>75%+ (Eligible)</option>
                <option value="50" <?php echo e(request('min_score')==='50'?'selected':''); ?>>50%+ (Review)</option>
            </select></div>
            ...
            <a href="<?php echo e(route('admin.ai.index')); ?>" class="btn btn-o btn-sm">Clear</a>
        </form>
    </div>
</div>
...
<div class="card an">
    <div class="ch">
        ...
        <span class="badge b-p" style="margin-left:6px;"><?php echo e($applications->total()); ?></span>
    ...
    <div class="tw"><table>
        ...
        <tbody>
        <?php $__empty_1 = true; $__currentLoopData = $applications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rank => $app): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php
            $sc  = $app->ai_score ?? 0;
        ...
        ?>
        <tr style="<?php echo e($rowBg); ?>">
            <td style="text-align:center;">
                <div style="width:30px;height:30px;border-radius:50%;background:<?php echo e($absRank<=3?'linear-gradient(135deg,#1a6b2f,#2d9e4f)':($sc>=75?'#d0f0d8':($sc>=50?'#fef3cd':'#fde8e6'))); ?>;color:<?php echo e($absRank<=3?'#f0c020':($sc>=75?'#0d6624':($sc>=50?'#a07c00':'#c0392b'))); ?>;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12px;margin:0 auto;"><?php echo e($absRank); ?></div>
            </td>
            <td>
                <div style="display:flex;align-items:center;gap:9px;">
                    <div class="av av-s"><?php echo e(strtoupper(substr($app->student?->user?->name??'?',0,1))); ?></div>
                    <div>
                        <div class="fws" style="font-size:13px;"><?php echo e($app->student?->user?->name ?? 'N/A'); ?></div>
                        <div style="font-size:11px;color:var(--tm);"><?php echo e($app->student?->course ?? ''); ?> · <?php echo e($app->student?->year_level ?? ''); ?></div>
                    </div>
                </div>
            </td>
            <td>
                <div class="fws" style="font-size:12px;"><?php echo e(\Illuminate\Support\Str::limit($app->scholarship?->name??'N/A',28)); ?></div>
                <span class="badge <?php echo e($app->scholarship?->type==='Government'?'b-p':($app->scholarship?->type==='Private'?'b-s':'b-i')); ?>" style="font-size:10px;"><?php echo e($app->scholarship?->type); ?></span>
            </td>
            <td class="mono fwb" style="color:var(--g);"><?php echo e(number_format($app->gwa,2)); ?></td>
            <td style="min-width:120px;">
                <div style="display:flex;align-items:center;gap:7px;">
                    <div style="flex:1;"><div class="asb"><div class="asf <?php echo e($sfC); ?>" style="width:<?php echo e($sc); ?>%;"></div></div></div>
                    <span class="mono fwb" style="font-size:13px;color:<?php echo e($sc>=75?'var(--gm)':($sc>=50?'var(--warn)':'var(--danger)')); ?>"><?php echo e($sc); ?>%</span>
                </div>
            </td>
            <td>
                <span class="badge elig-<?php echo e($elC); ?>"><?php echo e($el); ?></span>
                <?php if($app->ai_tag): ?><br><span style="font-size:10px;color:var(--tm);margin-top:2px;display:block;"><?php echo e($app->ai_tag); ?></span><?php endif; ?>
            </td>
            <td style="max-width:180px;font-size:11px;color:var(--tm);line-height:1.5;"><?php echo e(\Illuminate\Support\Str::limit($app->ai_reasoning??'—',85)); ?></td>
            <td><span class="badge <?php echo e($app->status==='Approved'?'b-s':($app->status==='Rejected'?'b-d':'b-w')); ?>"><?php echo e($app->status); ?></span></td>
            <td>
                <div style="display:flex;gap:4px;">
                    <a href="<?php echo e(route('admin.applications.show',$app->id)); ?>" class="btn btn-o btn-sm btn-ic" title="View"><i class="fas fa-eye"></i></a>
                    <?php if($app->status==='Pending'): ?>
                    <form method="POST" action="<?php echo e(route('admin.applications.approve',$app->id)); ?>"><?php echo csrf_field(); ?>
                        <button class="btn btn-s btn-sm btn-ic" title="Approve (AI Score: <?php echo e($sc); ?>%)"><i class="fas fa-check"></i></button>
                    </form>
                    <form method="POST" action="<?php echo e(route('admin.applications.reject',$app->id)); ?>"><?php echo csrf_field(); ?>
    ...
    <div style="padding:13px 18px;border-top:1px solid var(--bd);"><?php echo e($applications->appends(request()->query())->links()); ?></div>
    <?php endif; ?>
</div>


<div id="jsAppData" style="display:none;"><?php echo e(json_encode($applications->map(function($a){
    return [
...
<div id="jsStatsData" style="display:none;"><?php echo e(json_encode($stats)); ?></div>

<div id="jsSchData" style="display:none;"><?php echo e(json_encode($scholarships->map(function($s){
    return [
        'id'   => $s->id,
        'name' => $s->name,
        'type' => $s->type ?? '',
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Acer\Herd\isams\resources\views/admin/ai/index.blade.php ENDPATH**/ ?>